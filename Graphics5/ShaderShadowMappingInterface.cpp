 ...
 */
void ShaderShadowMappingInterface::SetPPC(PPC* ppc, float zNear, float zFar) {
	this->ppc = ppc;
	this->zNear = zNear;
	this->zFar = zFar;
}

void ShaderShadowMappingInterface::BuildBiasMatrix(M44& biasMatrix) {
	biasMatrix[0][0] = 0.5f;
	biasMatrix[0][1] = 0.0f;
	biasMatrix[0][2] = 0.0f;
	biasMatrix[0][3] = 0.5f;
...
void ShaderShadowMappingInterface::BuildLightViewMatrix(PPC* ppc, M44& lightViewMatrix) {
...
void ShaderShadowMappingInterface::BuildProjectionMatrix(PPC* ppc, float zNear, float zFar, M44& perspectiveMatrix) {
